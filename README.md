...
머신러닝 알고리즘을 IoT 디바이스의 동작에 활용하기 위해서 서버 API를 이용하면, 1) 디바이스는 항상 네트워크에 접속이 가능하여야 하고, 2) 디바이스의 숫자가 증가하면 서버의 처리 용량이 동일하게 증가되어야 하고, 3) 추론을 위한 API 호출 비용으로 ML을 활용하는데 제한 요소가 될 수 있습니다. 
